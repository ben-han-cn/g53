...
func (a *WA) Compare(other_ Rdata) int {
	other := other_.(*WA)
	order := fieldCompare(RDF_C_UINT16, a.Priority, other.Priority)
	if order != 0 {
		return order
	}
...
func (a *WA) String() string {
	return strings.Join([]string{
		fieldToString(RDF_D_INT, a.Priority),
...
func WAFromWire(buf *util.InputBuffer, ll uint16) (*WA, error) {
	...
	if err != nil {
		...
		host, _ := f.(net.IP)
		return &WA{priority, weight, host}, nil
	}
}

var waRdataTemplate = regexp.MustCompile(`^\s*(\S+)\s+(\S+)\s+(\S+)\s*$`)
...
func WAFromString(s string) (*WA, error) {
	...
	if err == nil {
		host, _ := f.(net.IP)
		return &WA{uint16(priority), uint16(weight), host.To4()}, nil
	} else {
		return nil, err
	}
}
